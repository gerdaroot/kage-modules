...
class Inactive(loader.Module):
    ...
    strings = {
        ...
        "processing_failed": "🚫 <b>Processing failed, check logs for details</b>",
        "hrs": "hour(-s)",
    ...
    strings_ru = {
        ...
        "processing_failed": "🚫 <b>Обработка не удалась, подробности в логах</b>",
        "hrs": "час(-ов)",
    ...
    async def _configure(self, call: InlineCall):
        await self.lookup("KageConfig").inline__configure(
            call,
    ...
    async def _start(self, call: InlineCall, chat_id: int):
        try:
            await self._collect(call, chat_id)
        except Exception:
            logger.exception("Inactive users scan failed in %s", chat_id)
            self._lock.pop(chat_id, None)
            await call.edit(self.strings("processing_failed"))

    async def _collect(self, call: InlineCall, chat_id: int):
        if chat_id in self._lock:
        ...
        async with self._global_lock:
            ...
            task = asyncio.ensure_future(report_progress())
            ...
            async with self._client.takeout(
                **(
                ...
                async for user in takeout.iter_participants(chat):
                    # Never kick yourself or other admins, whatever their activity.
                    if user.is_self or isinstance(
                        getattr(user, "participant", None), ADMIN_PARTICIPANTS
                    ):
                        continue
    ...
    async def _restrict(
        ...
        for user_id in restrict:
            try:
                await self._apply_action(chat_id, user_id)
            except Exception:
                logger.warning("Can't restrict %s in %s", user_id, chat_id, exc_info=True)
    ...
    async def _apply_action(self, chat_id: int, user_id: int):
        if self.config["action"] == "kick":
            await self._client.kick_participant(chat_id, user_id)
            return

        # Revoking view_messages is how Telegram represents a ban.
