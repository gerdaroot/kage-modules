...
class TicTacToeMod(loader.Module):
    ...
    async def _display_name(self, user_id: int) -> str:
        # Users who only interacted with the inline bot may be unknown to the userbot session
        try:
            return get_display_name(await self._client.get_entity(user_id))
        except ValueError:
            return str(user_id)

    async def _game_exists(self, call: InlineCall) -> bool:
        if call.form.get("uid") in self._games:
            return True
    ...
    async def _edit_board(self, call: InlineCall, render) -> None:
        rendered = render(call.form["uid"])
        if rendered:
            await call.edit(**rendered)
    ...
    async def _process_click(
        ...
        if not await self._game_exists(call):
            return
        ...
        await self._edit_board(call, self._render)

    async def _process_click_ai(self, call: InlineCall, i: int, j: int, line: str):
        if not await self._game_exists(call):
            return

        if call.from_user.id != self._games[call.form["uid"]]["user_id"]:
            await call.answer(self.strings("not_your_game"))
            return
        ...
        board = game["board"]
        board.make_move(i, j, game["human_player"])

        if not board.has_winner() and board.get_legal_moves():
            board.make_move(*game["bot"].select_move(board), game["ai_player"])

        await self._edit_board(call, self._render_ai)
...
{board[2][0]} | {board[2][1]} | {board[2][2]}"""
    ...
    async def inline__start_game(self, call: InlineCall):
        ...
        self._games[uid] = {
            ...
            "name": utils.escape_html(await self._display_name(call.from_user.id)),
            "score": "...|...|...",
        }
    ...
    async def inline__start_game_ai(self, call: InlineCall):
        ...
        first = choice(["bear", user_id])
        self._games[uid] = {
            "2_player": "bear",
            "turn": user_id,
            "mapping": {first: "x", "bear" if first != "bear" else user_id: "o"},
            "amifirst": first == user_id,
            "user_id": user_id,
            "user_name": utils.escape_html(await self._display_name(user_id)),
    ...
    def _render_ai(self, uid: str) -> dict:
        ...
        if win_o or win_x:
            ...
            return {
                "text": self.strings("win").format(
                    "🐻 Bear" if winner == "bear" else game["user_name"],
                    "❌" if win_x else "⭕️",
                    self._render_text(score),
                )
            }
