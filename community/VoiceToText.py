...
class VoiceToTextMod(loader.Module):
    ...
    async def _to_flac(self, src_path: str) -> bytes:
        rc, stdout, stderr = await self._run(
            "ffmpeg", "-y", "-i", src_path,
            "-ar", "16000", "-ac", "1", "-f", "flac", "pipe:1",
        )
    ...
    async def _transcribe_media(self, target: Message) -> str:
        """Raises FileNotFoundError without ffmpeg and RuntimeError when every provider fails."""
        with tempfile.TemporaryDirectory() as tmpdir:
            src_path = await self.client.download_media(target, file=os.path.join(tmpdir, "input"))
            flac_bytes = await self._to_flac(src_path)

        return await self._transcribe(flac_bytes)
    ...
    async def v2t(self, message: Message):
        ...
        try:
            text = await self._transcribe_media(reply)
        except FileNotFoundError:
            await utils.answer(message, self.strings["ffmpeg_missing"])
            return
        except RuntimeError as e:
            await utils.answer(message, self.strings["all_failed"].format(err=utils.escape_html(str(e))))
            return
    ...
    async def v2tlist(self, message: Message):
        ...
        lines.extend(
            ...
            for index, (chat_id, name) in enumerate(zip(selected, names, strict=True), start=1)
        )
        if pages > 1:
            lines.extend((
                "",
    ...
    async def watcher(self, message: Message):
        ...
        duration = message.file.duration if message.file else None
        if duration and duration > self.config["max_duration"]:
            return

        # Errors are only logged: answering here would post them into someone else's chat
        try:
            text = await self._transcribe_media(message)
        except Exception:
            logger.exception("VoiceToText auto-transcription failed")
            return
