...
# Kage's own service chats (kage-userbot forum, logs) plus names inherited from Heroku/Hikka
SERVICE_CHAT_PREFIXES = ("kage-", "heroku-", "hikka-")
...
class MentionsTaggerMod(loader.Module):
    ...
    async def client_ready(self):
        self.heroku_forum = self._db.get("kage.forums", "channel_id", 0)
        self.assets_topic = None
        if not self.heroku_forum:
            logger.warning("Kage content channel is not set up, mention alerts are disabled")
            return
    ...
    async def watcher(self, message: Message):
        if not self.get("active", False) or not self.assets_topic:
            return
        ...
        if (
            chat_title.lower().startswith(SERVICE_CHAT_PREFIXES)
            or chat_username.lower().startswith(SERVICE_CHAT_PREFIXES)
        ):
            return

        me = self.client.kage_me
        username = me.username or (
            me.usernames[0].username if getattr(me, "usernames", None) else f"id{self.tg_id}"
        )

        display_name = utils.escape_html(self.config["display_name"] or self.strings("user_label"))
    ...
    def _add_ignored(self, key: str, entity_id: int):
        if entity_id not in self.config[key]:
            self.config[key] = self.config[key] + [entity_id]

    async def _handle_ignore(self, call: BotInlineCall, target: str, chat_id: int, user_id: int):
        match target:
            case "chat":
                self._add_ignored("ignored_chats", chat_id)
                await call.answer(self.strings("alert_chat_ignored"), show_alert=True)

            case "user":
                self._add_ignored("ignored_users", user_id)
