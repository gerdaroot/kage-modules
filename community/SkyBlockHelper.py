...
class SkyBlockHelperMod(loader.Module):
    ...
    def _display(self, text: str) -> str:
        return utils.escape_html(self._strip_formatting(text))
    ...
    async def _fetch_json(self, path: str, timeout: int = 10) -> dict | None:
        """GET a keyless Hypixel API endpoint, None on any failure"""
        try:
            async with aiohttp.ClientSession(
                timeout=aiohttp.ClientTimeout(total=timeout)
            ) as session:
                async with session.get(HYPIXEL_API + path) as resp:
                    if resp.status != 200:
                        return None
    ...
    async def _fetch_bazaar(self) -> dict | None:
        data = await self._fetch_json("skyblock/bazaar")
        return data.get("products") if data else None

    async def _fetch_auctions(self) -> list[dict] | None:
        # Only the first of ~40 pages: fetching all of them would take too long.
        data = await self._fetch_json("skyblock/auctions?page=0", timeout=12)
        return data.get("auctions") if data else None

    async def _get_all_items(self) -> list[dict] | None:
        if not self._items_cache:
            data = await self._fetch_json("resources/skyblock/items", timeout=15)
            self._items_cache = data.get("items") if data else None
        return self._items_cache

    async def _search_bin_auctions(self, query: str) -> list[dict] | None:
        """Active BIN auctions matching query, cheapest first; None on API error"""
        ...
        matched = [
            auc
            for auc in auctions
            if auc.get("bin", False)
            and clean_q in self._strip_formatting(auc.get("item_name", "")).lower()
        ]
        return sorted(matched, key=lambda x: x.get("starting_bid", 0))
    ...
    async def _build_mayor_card(self) -> tuple[str, list[list[dict]]] | tuple[None, None]:
        ...
        data = await self._fetch_json("resources/skyblock/election")
        if not data:
            return None, None

        mayor = data.get("mayor", {})
        name = self._display(mayor.get("name", "Unknown"))
        perks_list = mayor.get("perks", [])

        formatted_perks = ""
        for perk in perks_list:
            p_name = self._display(perk.get("name", "Unknown Perk"))
            p_desc = self._display(perk.get("description", ""))
        ...
        if candidates:
            ...
            for c in sorted(candidates, key=lambda x: x.get("votes", 0), reverse=True):
                c_name = self._display(c.get("name", "Candidate"))
    ...
    async def _build_firesale_card(self) -> tuple[str, list[list[dict]]] | tuple[None, None]:
        ...
        data = await self._fetch_json("skyblock/firesales")
        if not data:
            return None, None
        ...
        for sale in sales:
            item_id = utils.escape_html(sale.get("item_id", "Unknown"))
    ...
    def _build_ah_card(self, query: str, matched: list[dict]) -> tuple[str, list[list[dict]]]:
        ...
        lot_lines = []
        for lot in top_lots:
            name = self._display(lot.get("item_name", "Item"))
    ...
    async def _get_report_topic(self) -> tuple[int, int] | None:
        """Chat id and topic id of the module's topic in the Kage forum channel"""
        channel_id = self._db.get("kage.forums", "channel_id", None)
        if not channel_id:
            return None
        ...
        return int(f"-100{channel_id}"), topic.id

    def _build_hourly_report(self, products: dict) -> str | None:
        coins_unit = self.strings("coins_unit")
        lines = [
            f"• <code>{item_id}</code>: Buy <code>{qs.get('buyPrice', 0.0):,.1f}</code>"
            f" | Sell <code>{qs.get('sellPrice', 0.0):,.1f}</code> {coins_unit}"
            for item_id in self.config["tracked_items"]
            if item_id in products
            for qs in [products[item_id].get("quick_status", {})]
        ]
        return self.strings("hourly_report_header") + "\n".join(lines) if lines else None

    async def _send_hourly_report(self):
        if not self.config["tracked_items"]:
            return
        products = await self._fetch_bazaar()
        if not products or not (report := self._build_hourly_report(products)):
            return
        if not (target := await self._get_report_topic()):
            return
    ...
    async def _hourly_tracker_loop(self):
        ...
        while True:
            ...
            try:
                ...
                logger.exception("SkyBlock hourly report failed")
    ...
    async def bzcmd(self, message: Message):
        ...
        if not item_id:
            return await utils.answer(
                message, self.strings("not_found").format(query=utils.escape_html(query))
            )
    ...
    async def bztrackcmd(self, message: Message):
        ...
        if not item_id:
            return await utils.answer(
                message, self.strings("not_found").format(query=utils.escape_html(args))
            )
    ...
    async def bzuntrackcmd(self, message: Message):
        ...
        if not target_item:
            return await utils.answer(
                message,
                self.strings("track_not_in_list").format(item=utils.escape_html(args)),
            )
    ...
    async def sbitemcmd(self, message: Message):
        ...
        name = self._display(matched_item.get("name", matched_item.get("id", "Unknown")))
    ...
    async def sbahcmd(self, message: Message):
        ...
        matched = await self._search_bin_auctions(query)
        if matched is None:
            return await utils.answer(message, self.strings("api_error"))
        if not matched:
            return await utils.answer(
                message, self.strings("ah_empty").format(query=utils.escape_html(query))
            )
    ...
    async def _handle_refresh_ah(self, call: BotInlineCall, query: str):
        ...
        if not matched:
            return await call.edit(
                self.strings("ah_empty").format(query=utils.escape_html(query))
            )
