...
class UploaderMod(loader.Module):
    ...
    async def _up_pixeldrain(self, file_bytes: bytes, filename: str) -> str:
        ...
        async with self._session() as session:
            async with session.put(
                f"https://pixeldrain.com/api/file/{quote(filename)}", data=file_bytes, auth=auth
            ) as resp:
                body = await resp.json(content_type=None)
                file_id = body.get("id")
                if not file_id:
    ...
    async def _up_gofile(self, file_bytes: bytes, filename: str) -> str:
        ...
        async with self._session() as session:
            ...
            async with session.post(
                f"https://{server}.gofile.io/contents/uploadfile", data=data, headers=headers
            ) as resp:
                body = await resp.json(content_type=None)
    ...
    async def _up_filebin(self, file_bytes: bytes, filename: str) -> str:
        url = f"https://filebin.net/{_rand_bin()}/{quote(filename)}"
        async with self._session() as session:
            async with session.post(
                url,
                data=file_bytes,
    ...
    async def _run_upload(self, message: Message, service: str, upload_fn, key_field: str | None = None):
        ...
        file_bytes = await self.client.download_media(target, bytes)
        if not file_bytes:
            await utils.answer(message, self.strings["no_file"])
            return
        filename = target.file.name or f"file{target.file.ext or ''}"

        try:
            url = await upload_fn(file_bytes, filename)
        except Exception as e:
            err = utils.escape_html((str(e) or repr(e))[:300])
            await utils.answer(message, self.strings["failed"].format(service=service, err=err))
            return

        await utils.answer(message, self.strings["result"].format(service=service, url=utils.escape_html(url)))
